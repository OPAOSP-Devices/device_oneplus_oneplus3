...
HELPER="$OPAOSP_ROOT"/vendor/opaosp/build/tools/extract_utils.sh
if [ ! -f "$HELPER" ]; then
...
setup_vendor "$DEVICE" "$VENDOR" "$OPAOSP_ROOT" false "$CLEAN_VENDOR"

extract "$MY_DIR"/proprietary-files.txt "$SRC" "$SECTION"

"$MY_DIR"/setup-makefiles.sh

DEVICE_BLOB_ROOT="$OPAOSP_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary
